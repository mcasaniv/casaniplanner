...
const getCourseOfMateria = (materia: Materia): Curso | 'Otro' => {
  for (const [curso, materias] of Object.entries(CURSOS_MATERIAS)) {
...
const getMateriaStyles = (materia: Materia, isCompleted: boolean) => {
  ...
  switch (course) {
    case 'Ciencias':
      return {
        card: isCompleted 
          ? 'bg-emerald-500 text-white border-emerald-650 dark:bg-emerald-650 dark:border-emerald-550 ring-2 ring-emerald-500/20' 
          : 'bg-emerald-50/70 hover:bg-emerald-100/70 text-emerald-800 border-emerald-200 dark:bg-emerald-950/20 dark:text-emerald-300 dark:border-emerald-900/50 dark:hover:border-emerald-800/80',
        checkbox: isCompleted
          ? 'bg-emerald-650 border-emerald-650 text-white'
          : 'border-emerald-300 dark:border-emerald-800',
        text: isCompleted ? 'text-white line-through font-bold' : 'text-emerald-900 dark:text-emerald-200',
        badge: 'bg-emerald-500/20 text-emerald-700 dark:bg-emerald-500/10 dark:text-emerald-400 border-emerald-200/50 hover:bg-emerald-500/30 dark:hover:bg-emerald-500/20'
      };
    case 'Matemática':
      return {
        card: isCompleted 
          ? 'bg-amber-500 text-slate-900 border-amber-600 dark:bg-amber-600 dark:border-amber-500 dark:text-white ring-2 ring-amber-500/20' 
          : 'bg-amber-50/60 hover:bg-amber-100/60 text-amber-800 border-amber-200 dark:bg-amber-950/10 dark:text-amber-305 dark:border-amber-900/50 dark:hover:border-amber-800/80',
        checkbox: isCompleted
          ? 'bg-amber-600 border-amber-600 text-white dark:bg-amber-500'
          : 'border-amber-300 dark:border-amber-800',
        text: isCompleted ? 'text-slate-900 line-through font-bold dark:text-white' : 'text-amber-900 dark:text-amber-200',
        badge: 'bg-amber-500/20 text-amber-700 dark:bg-amber-500/10 dark:text-amber-400 border-amber-200/50 hover:bg-amber-500/30 dark:hover:bg-amber-500/20'
      };
    case 'Comunicación':
      return {
        card: isCompleted 
          ? 'bg-rose-500 text-white border-rose-600 dark:bg-rose-600 dark:border-rose-500 ring-2 ring-rose-500/20' 
          : 'bg-rose-50/70 hover:bg-rose-100/70 text-rose-800 border-rose-200 dark:bg-rose-950/20 dark:text-rose-300 dark:border-rose-900/50 dark:hover:border-rose-800/80',
        checkbox: isCompleted
          ? 'bg-rose-600 border-rose-600 text-white'
          : 'border-rose-300 dark:border-rose-800',
        text: isCompleted ? 'text-white line-through font-bold' : 'text-rose-900 dark:text-rose-200',
    ...
    case 'Sociales':
      return {
        card: isCompleted 
          ? 'bg-blue-500 text-white border-blue-600 dark:bg-blue-600 dark:border-blue-550 ring-2 ring-blue-500/20' 
          : 'bg-blue-50/70 hover:bg-blue-100/70 text-blue-800 border-blue-200 dark:bg-blue-950/20 dark:text-blue-300 dark:border-blue-900/50 dark:hover:border-blue-800/80',
        checkbox: isCompleted
          ? 'bg-blue-600 border-blue-600 text-white'
          : 'border-blue-300 dark:border-blue-800',
        text: isCompleted ? 'text-white line-through font-bold' : 'text-blue-900 dark:text-blue-200',
        badge: 'bg-blue-500/20 text-blue-700 dark:bg-blue-500/10 dark:text-blue-400 border-blue-200/50 hover:bg-blue-500/30 dark:hover:bg-blue-500/20'
      };
    case 'DPCC':
      return {
        card: isCompleted 
          ? 'bg-orange-500 text-white border-orange-650 dark:bg-orange-600 dark:border-orange-550 ring-2 ring-orange-500/20' 
          : 'bg-orange-50/70 hover:bg-orange-100/70 text-orange-850 border-orange-200 dark:bg-orange-950/20 dark:text-orange-300 dark:border-orange-900/50 dark:hover:border-orange-800/80',
        checkbox: isCompleted
          ? 'bg-orange-600 border-orange-650 text-white'
          : 'border-orange-300 dark:border-orange-850',
        text: isCompleted ? 'text-white line-through font-bold' : 'text-orange-900 dark:text-orange-200',
        badge: 'bg-orange-500/20 text-orange-700 dark:bg-orange-500/10 dark:text-orange-400 border-orange-200/50 hover:bg-orange-500/30 dark:hover:bg-orange-500/20'
      };
    case 'Razonamiento':
      return {
        card: isCompleted 
          ? 'bg-sky-500 text-white border-sky-600 dark:bg-sky-600 dark:border-sky-550 ring-2 ring-sky-500/20' 
          : 'bg-sky-50/70 hover:bg-sky-100/70 text-sky-850 border-sky-200 dark:bg-sky-950/20 dark:text-sky-300 dark:border-sky-900/50 dark:hover:border-sky-800/80',
        checkbox: isCompleted
          ? 'bg-sky-650 border-sky-650 text-white'
          : 'border-sky-300 dark:border-sky-800',
        text: isCompleted ? 'text-white line-through font-bold' : 'text-sky-900 dark:text-sky-200',
        badge: 'bg-sky-500/20 text-sky-700 dark:bg-sky-500/10 dark:text-sky-400 border-sky-200/50 hover:bg-sky-500/30 dark:hover:bg-sky-500/20'
      };
    default:
      return {
        card: isCompleted 
          ? 'bg-violet-500 text-white border-violet-600 dark:bg-violet-600 dark:border-violet-550 ring-2 ring-violet-500/20' 
          : 'bg-violet-50/70 hover:bg-violet-100/70 text-violet-850 border-violet-200 dark:bg-violet-950/20 dark:text-violet-300 dark:border-violet-900/50 dark:hover:border-violet-800/80',
        checkbox: isCompleted
          ? 'bg-violet-650 border-violet-650 text-white'
          : 'border-violet-300 dark:border-violet-800',
        text: isCompleted ? 'text-white line-through font-bold' : 'text-violet-900 dark:text-violet-200',
        badge: 'bg-violet-500/20 text-violet-700 dark:bg-violet-500/10 dark:text-violet-400 border-violet-200/50 hover:bg-violet-500/30 dark:hover:bg-violet-500/20'
      };
  }
};
...
export default function App() {
  ...
  const [scheduleActiveTab, setScheduleActiveTab] = useState<'morning' | 'afternoon'>('morning');
  ...
  React.useEffect(() => {
    ...
    if (dailyChallenges.date !== todayStr || hasDuplicateTopics) {
      ...
      const todayAfternoonCourses = schedule.filter(s => s.day === todayDayName && s.section === 'afternoon');
      const hasAfternoonCourses = todayAfternoonCourses.length > 0;
      const allAfternoonCompleted = hasAfternoonCourses && todayAfternoonCourses.every(entry => entry.completedDates?.includes(todayStr));

      let challengesCompleted = false;
      if (dailyChallenges.taskIds.length > 0) {
        challengesCompleted = dailyChallenges.taskIds.every(id => {
          const task = tasks.find(t => t.id === id);
          return !task || task.status === 'completed'; // If deleted or completed
        });
      }
      
      if ((challengesCompleted || allAfternoonCompleted) && lastStreakDate !== todayStr) {
        setStreak(s => s + 1);
  ...
  const [newScheduleEntry, setNewScheduleEntry] = useState<{ day: DayOfWeek; materia: Materia; section: 'morning' | 'afternoon' }>({
    ...
    return (
      <div className="space-y-6">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            <h2 className="text-2xl font-bold tracking-tight text-slate-900 dark:text-slate-100">Horarios de Estudio</h2>
            <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">
              Organiza tus clases de la mañana y tu estudio de la tarde por tu cuenta.
            </p>
          </div>
          
          <div className="flex bg-slate-100 dark:bg-slate-800 p-1 rounded-xl w-fit">
            <button
              onClick={() => setScheduleActiveTab('morning')}
              className={`flex items-center gap-2 px-4 py-2 text-sm font-semibold rounded-lg transition-all ${
                scheduleActiveTab === 'morning'
                  ? 'bg-white dark:bg-slate-900 text-slate-900 dark:text-slate-100 shadow-sm'
                  : 'text-slate-500 hover:text-slate-900 dark:hover:text-slate-100'
              }`}
            >
              <Sun className="w-4 h-4 text-amber-500" />
              <span>Clases de la Mañana</span>
            </button>
            <button
              onClick={() => setScheduleActiveTab('afternoon')}
              className={`flex items-center gap-2 px-4 py-2 text-sm font-semibold rounded-lg transition-all ${
            ...
            >
              ...
              <span>Estudio de la Tarde</span>
            </button>
          </div>
        </div>

        {scheduleActiveTab === 'afternoon' && (
          <div className="p-4 bg-emerald-50 dark:bg-emerald-950/20 border border-emerald-100 dark:border-emerald-900/30 rounded-xl flex items-center gap-3">
            <div className="p-2 bg-emerald-500 text-white rounded-lg">
              <Sparkles className="w-5 h-5 animate-pulse" />
            </div>
            <div>
              <p className="text-sm font-bold text-emerald-800 dark:text-emerald-300">¡Ruta de Racha Alternativa!</p>
              <p className="text-xs text-emerald-600 dark:text-emerald-400">
                Si completas las materias programadas para hoy por la tarde, ¡obtendrás/mantendrás tu racha diaria!
                (Haz clic sobre una materia de la tarde para colorearla y marcarla como estudiada hoy).
              </p>
            </div>
          </div>
        )}
        ...
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
          {days.map(day => {
            ...
            return (
              <Card key={day} className={`flex flex-col h-full border ${isToday ? 'ring-2 ring-emerald-500 dark:ring-emerald-400 border-transparent bg-slate-50/40 dark:bg-slate-900/40' : 'border-slate-200 dark:border-slate-800'}`}>
                <CardHeader className={`pb-2 border-b transition-colors ${
                  isToday 
                ...
                <CardContent className="pt-4 flex-1 flex flex-col gap-2">
                  ...
                  {dayEntries.length === 0 && (
                    <p className="text-xs text-slate-400 italic text-center py-4">No programado</p>
                  )}
                  ...
                  <Button 
                    ...
                    onClick={() => {
                      setNewScheduleEntry({ ...newScheduleEntry, day, section: scheduleActiveTab });
  ...
  return (
    <div className="h-screen overflow-hidden bg-slate-50 dark:bg-slate-950 flex flex-col md:flex-row font-sans text-slate-900 dark:text-slate-100 transition-colors duration-200">
      ...
      <Modal isOpen={isScheduleModalOpen} onClose={() => setIsScheduleModalOpen(false)} title={`Agregar a ${newScheduleEntry.day}`}>
        <form onSubmit={(e) => {
          e.preventDefault();
          addScheduleEntry(newScheduleEntry.day, newScheduleEntry.materia, newScheduleEntry.section);
          ...
          <div>
            <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">Momento del día</label>
            <Select 
              value={newScheduleEntry.section} 
              onChange={(e) => setNewScheduleEntry({ ...newScheduleEntry, section: e.target.value as 'morning' | 'afternoon' })}
            >
              <option value="morning">Mañana (Clases)</option>
              <option value="afternoon">Tarde (Estudio por mi cuenta)</option>
            </Select>
          </div>
          <div className="pt-2 flex justify-end gap-2">
